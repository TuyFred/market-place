...
/* ─── Types ─── */
type Product = { id: string; name: string; description?: string; price: number; category: string; size?: string; imageUrl?: string; imageUrls?: string[]; stock?: number; is_active?: boolean };
    ...
    <div className={`${containerClass} app-shell fixed inset-0 flex`}>
      ...
      <aside className={`${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 fixed md:static inset-y-0 left-0 z-40 w-64 flex flex-col transition-transform duration-300 ease-in-out sidebar ${theme === 'dark' ? 'glass-dark' : 'bg-white border-r border-slate-100'}`}>
        <div className="h-16 flex items-center gap-3 px-6 border-b" style={theme === 'dark' ? { borderColor: 'rgba(148,163,184,.06)' } : { borderColor: 'rgba(2,6,23,.04)' }}>
          ...
          <span className={`${theme === 'dark' ? 'text-white' : 'text-slate-900'} font-semibold tracking-tight`}>AFROLUXO Admin</span>
              ...
              <div className="rounded-2xl p-5 border card">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  ...
                  <select className={inputClass} value={prodForm.category || ''} onChange={(e) => setProdForm({ ...prodForm, category: e.target.value })}>
                    <option value="">Select clothes category</option>
                  ...
                  <input className={inputClass} placeholder="Size (e.g. S, M, L, XL or 38)" value={prodForm.size || ''} onChange={(e) => setProdForm({ ...prodForm, size: e.target.value })} />
                  ...
                  <input className={inputClass + ' md:col-span-2'} placeholder="Image URL (or pick a sample below)" value={prodForm.imageUrl || ''} onChange={(e) => setProdForm({ ...prodForm, imageUrl: e.target.value })} />
                  <div className="md:col-span-2">
                    <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">Sample images (replace later)</p>
                    <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
                      {Object.entries(SAMPLE_IMAGES).filter(([k]) => k !== 'hero' && k !== 'placeholder').map(([key, url]) => (
                        <button
                          key={key}
                          type="button"
                          title={`Use ${key}`}
                          onClick={() => setProdForm({
                            ...prodForm,
                            imageUrl: url,
                            imageUrls: [url, ...(prodForm.imageUrls || []).filter((u) => u !== url)].slice(0, 6)
                          })}
                          className={`relative w-16 h-16 rounded-xl overflow-hidden border-2 shrink-0 transition-all ${prodForm.imageUrl === url ? 'border-amber-400 ring-2 ring-amber-400/30' : 'border-slate-600 hover:border-slate-400'}`}
                        >
                  ...
                  <div className="md:col-span-2 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {(prodForm.imageUrls?.length ? prodForm.imageUrls : (prodForm.imageUrl ? [prodForm.imageUrl] : [])).map((u, i) => (
                      ...
                      {!prodForm.imageUrl && !(prodForm.imageUrls && prodForm.imageUrls.length) && (
                        <div className="w-20 h-20 rounded-xl overflow-hidden border border-dashed border-slate-600 opacity-60">
                          <img src={SAMPLE_IMAGES.placeholder} alt="Sample" className="w-full h-full object-cover" />
                        </div>
                      )}
                      <label className="w-20 h-20 rounded-xl border-2 border-dashed border-slate-700 flex flex-col items-center justify-center cursor-pointer hover:border-forest hover:bg-forest/5 transition-all text-slate-300">
                        <svg className="h-5 w-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" /></svg>
                        <span className="text-[10px] font-bold">Upload</span>
                        <input type="file" className="hidden" onChange={(e) => uploadFiles(e.target.files)} multiple accept="image/*" />
                      </label>
                    </div>
                    {prodForm.imageUrl && (
                      <div className="text-[10px] text-slate-500 italic">
                        First gallery image is the primary thumbnail. You can replace samples with uploads anytime.
                      </div>
                    )}
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 mt-4">
                  <button
                    ...
                    onClick={saveProd}
                    disabled={loading}
                    className="inline-flex items-center gap-2 rounded-full px-5 py-2.5 text-xs font-bold uppercase tracking-wider text-white bg-ink hover:bg-forest transition-colors disabled:opacity-60"
                  >
                    {loading ? (
                      <>Saving...</>
                    ) : editingProd ? (
                  ...
                  {editingProd && <button type="button" onClick={() => { setEditingProd(null); setProdForm({}); }} className="text-xs text-slate-400 hover:text-white px-3">Cancel</button>}
                </div>
              </div>
                            ...
                            <div className="flex items-center gap-3">
                              <div className="relative group cursor-pointer" onClick={() => { setViewingProd(p); setViewingImageIdx(0); setViewingRotation(0); }}>
                                {p.imageUrl ? (
                                  ...
                                  <img src={SAMPLE_IMAGES.placeholder} className="w-10 h-10 rounded-xl object-cover border border-slate-700 opacity-80" alt="" />
                                )}
            ...
            <div className="space-y-6">
              <div className="rounded-2xl p-5 border card">
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  ...
                  <input className={inputClass} placeholder="Slug (e.g. african-traditional)" value={catForm.slug} onChange={(e) => setCatForm({ ...catForm, slug: e.target.value })} />
                </div>
                <div className="flex gap-2 mt-4">
