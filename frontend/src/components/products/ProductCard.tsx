    ...
    <>
      <article className="premium-card group relative flex flex-col">
        ...
        <div className="flex flex-1 flex-col p-6 sm:p-8">
          <div className="mb-2">
            <h3 className="line-clamp-2 text-lg sm:text-xl font-black text-slate-900 group-hover:text-indigo-600 transition-colors leading-[1.1] tracking-tight">
              {product.name}
            </h3>
          ...
          <div className="mt-6 space-y-3">
            <button
              ...
              className={`shimmer-on-hover w-full flex items-center justify-center gap-2 rounded-2xl py-4 text-[11px] font-black uppercase tracking-widest transition-all duration-500 active:scale-95 shadow-xl ${added
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="shimmer-on-hover group/btn flex items-center justify-center gap-2 rounded-2xl bg-[#ecfdf5] border border-emerald-100 px-3 py-3 text-[10px] font-black tracking-widest uppercase text-emerald-700 transition-all hover:bg-emerald-100 active:scale-95"
              >
              ...
              <button
                onClick={handleEmail}
                className="group/btn flex items-center justify-center gap-2 rounded-2xl bg-slate-50 border border-slate-100 px-3 py-3 text-[10px] font-black tracking-widest uppercase text-slate-700 transition-all hover:bg-slate-100 active:scale-95"
              >
              ...
              <div className="flex-1 relative bg-slate-50 flex flex-col min-h-[350px]">
                ...
                <div className="flex-1 flex items-center justify-center p-12 overflow-hidden relative">
                  ...
                  <img
                    ...
                    className="max-w-full max-h-full object-contain transition-all duration-700 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] rounded-[2rem] relative z-10"
              ...
              <div className="w-full lg:w-[450px] p-8 sm:p-10 flex flex-col gap-8 overflow-y-auto bg-white border-l border-slate-100">
                <div className="flex justify-between items-start">
                  <div className="space-y-4">
                    <span className="glass inline-flex items-center rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-[0.2em] text-indigo-600 shadow-lg shadow-indigo-500/5">
                      {product.category}
                    </span>
                    <h2 className="text-4xl font-black text-slate-900 leading-[1.05] tracking-tight">
                      {product.name}
                    </h2>
                  </div>
                  <button
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between p-8 rounded-[2.5rem] bg-slate-50 border border-slate-100 shadow-[inset_0_2px_10px_rgba(0,0,0,0.02)]">
                    <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em]">Price Card</span>
                ...
                <div className="mt-auto space-y-4 pt-6">
                  <button
                    onClick={handleAddToCart}
                    className={`shimmer-on-hover w-full flex items-center justify-center gap-3 rounded-2xl py-4 text-xs font-black tracking-wide transition-all duration-300 active:scale-95 shadow-xl ${added
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-6 border-t border-slate-100/50">
                    <button
                      onClick={handleWhatsApp}
                      className="flex items-center justify-center gap-3 rounded-[1.5rem] bg-[#ecfdf5] border border-emerald-100 px-4 py-4 text-[10px] font-black tracking-widest uppercase text-emerald-700 transition-all hover:bg-emerald-100 active:scale-95 shadow-lg shadow-emerald-500/5 group/btn"
                    >
                      <svg className="h-5 w-5 transition-transform group-hover/btn:scale-110" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                      </svg>
                      WhatsApp Chat
                    </button>
                    <button
                      onClick={handleEmail}
                      className="flex items-center justify-center gap-3 rounded-[1.5rem] bg-slate-50 border border-slate-100 px-4 py-4 text-[10px] font-black tracking-widest uppercase text-slate-700 transition-all hover:bg-slate-100 active:scale-95 shadow-lg shadow-slate-900/5 group/btn"
                    >
